...
public class EditStudentFrame extends JFrame {
	...
	public EditStudentFrame(Student tempStudent) {
		...
		contentPane.add(studentGradeComb);
		 ...
		 studentSecondaryComb.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent e) {
		 		EditStudentFrame.this.studentMajorComb.setModel(new DefaultComboBoxModel(CollegeStructure.major[ studentSecondaryComb.getSelectedIndex()]));//选择不同的学院显示对应专业
		 		setStudentClass(e);
		 	}
		 });
		...
		studentClassComb.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EditStudentFrame.this.studentClassId.setText(arrayClass.get(studentClassComb.getSelectedIndex()).getId());//选择不同班级显示对应编号
			}
		});
		...
		this.studentClassComb.setSelectedItem(tempStudent.getClassName());
	...
	protected void confirmButton(ActionEvent e) {
		...
		Student tempStudent1 =new Student(id, grade, classId,className, name, sex, nation,major, secondary);
		JOptionPane.showMessageDialog(this,new StudentDao().editStudentInfo(tempStudent1));
		if(ManagerFrame.studentlisframe!=null) {////当打开学生列表页面时再进行添加学生可以及时显示在列表上
